...
1. LLMBudget — денежный бюджет на LLM-инференс. При исчерпании генерация
   деградирует до retrieval-only черновика (см. docs/architecture.md,
   лестница деградации). Дополняет дневной лимит вызовов в DraftService:
   лимит вызовов — суточная политика, ₽-бюджет — защита от всплеска затрат.
2. IncidentDeduplicator — схлопывает похожие тикеты в кластеры инцидента.
   ...
   ответ БЕЗ повторного LLM-вызова (см. docs/architecture.md, дедупликация).

Упрощения PoC и чем заменяются в целевой архитектуре:
...
- дедуп — blocking по теме + Jaccard по токенам маскированного текста;
  в целевой системе — embeddings + LSH + streaming-кластеризация.
...
class IncidentDeduplicator:
    ...
    def active_clusters(self) -> int:
        """Число живых кластеров — для /health и мониторинга дедупа."""
